...
function prepareObjects(jsonData) {
  ...
  jsonData.forEach(jsonObject => {
    ...
    //student.middlename = texts.split(" "[1]);
    //student.nickname = texts.split(" "[2]);
    //student.lastname = texts.split(" "[3]);
  ...
  myname.firstname = "Andreea";
  myname.middlename = "Ioana";
  myname.lastname = "Mihai";
...
function displayStudent(student, index) {
  // create clone
  const clone = document.querySelector("template#myTemp").content.cloneNode(true);
...
function checkFamilyBloodS(familyBloodS, student) {
  ...
  if (familyBloodS.half.includes(capitalizeFirstLetter(student.lastname.toLowerCase()))) {
    bloodInfo.textContent = "halfblood";
  } else if (familyBloodS.pure.includes(capitalizeFirstLetter(student.lastname.toLowerCase()))) {
